// Links and information
// About (Header) Dropdown
export const LINKS = {
  сompany: '#',
  token: 'https://mybit.us15.list-manage.com/subscribe?u=af48b1fdb5278fd9884338f23&id=0f78f78616',//(links to token page)
  blog: 'https://medium.com/mybit-dapp',
  contact: 'mailto:info@mybit.io',
  events: 'https://www.facebook.com/pg/MyBitDApp/events/',
  ddf: 'https://ddf.mybit.io/',
  //Resources (Header) dropdown
  tokenSale: 'https://medium.com/mybit-dapp/mybit-token-distribution-phase-2-b938ee231496', //(no link yet)
  whitepaper: 'https://whitepaper.mybit.io',
  github: 'https://github.com/MyBitFoundation',
  devs: 'https://developer.mybit.io/portal/',

  subscribeLink: 'https://chimpstatic.com/mcjs-connected/js/users/af48b1fdb5278fd9884338f23/50e4f28ebe0a2cd4dd5290132.js',

  // Pages
  aboutPage: 'about-us#top',
  applications: 'applications#top',
  fund: 'fund#top',
  token: 'token#top',

  // News
  news: 'https://medium.com/mybit-dapp/introduction-a8275e6bcbec',

  //Social Links
  telegram: 'https://t.me/mybitio',
  facebook: 'https://www.facebook.com/MyBitDApp/',
  twitter: 'https://twitter.com/MyBit_DApp',
  medium: 'https://medium.com/mybit-dapp',
  reddit: 'https://www.reddit.com/user/MyBit_DApp/',
  youTube: 'https://www.youtube.com/channel/UCtLn7Vi-3VbsY5F9uF1RJYg',
  linkedin: 'https://www.linkedin.com/company/mybit/',

  //Media
  forbes: 'https://www.forbes.com/sites/omribarzilay/2017/08/14/why-blockchain-is-the-future-of-the-sharing-economy/#8a6994033422',
  nasdaq: 'https://www.nasdaq.com/article/blockchain-technology-could-disrupt-and-reboot-the-sharing-economy-cm836757',
  wsj: 'https://www.wsj.com/articles/switzerland-wants-to-be-the-world-capital-of-cryptocurrency-1524942058',
  express: 'https://www.express.co.uk/finance/city/915842/cryptocurrency-news-dow-jones-what-could-collapse-mean-bitcoin-ripple-ethereum',
  tnw: 'https://thenextweb.com/contributors/2017/09/21/blockchain-tech-missing-link-success-iot/',
  coinJournal: 'https://coinjournal.net/?s=mybit',
  cryptoInsider: 'https://cryptoinsider.21mil.com/enterprise-level-investing-possible-blockchain/',
  futureTechPodcast: 'https://www.futuretechpodcast.com/podcasts/mybit-io-a-platform-for-crowdfunding-machine-ownership-investments/',
  chainbits: 'https://www.chainbits.com/reviews/mybit-review/',
  everipedia: 'https://everipedia.org/wiki/mybit-token-cryptocurrency/',

  //Partners
  eAliance: 'https://entethalliance.org/members-2/',
  cryptoValley: 'https://cryptovalley.swiss/',
  cryptoValleyLabs: 'https://cryptovalleylabs.com',
  mll: 'http://www.mll-legal.com/home/',
  slockIt: 'https://slock.it/landing.html',
  ability: 'https://www.abilityconcept.eu/en/',
  alpineMining: 'https://alpinemining.ch/en/',

  //Exchanges
  idex: 'https://idex.market/eth/myb',
  laToken: 'https://wallet.latoken.com/market/Crypto/ETH/MYB-ETH',
  bancor: 'https://www.bancor.network/communities/5b16460462bc740001afa01e/currency',
  hitBTC: 'https://hitbtc.com/MYB-to-ETH',

  //Application Links
  mybitTrust: 'https://trust.mybit.io',
  mybitWill: 'https://will.mybit.io/',
  dropzone: 'https://dropzone.mybit.io/',
  fork: 'https://form.mybit.io/',
  mybitPayroll: 'https://payroll.mybit.io',
  mybitOptions: 'https://options.mybit.io',
  mybitGo: 'https://app.mybit.io/',
  androidMobileWallet: 'https://play.google.com/store/apps/details?id=io.mybit.app',
  iosMobileWallet: 'https://itunes.apple.com/WebObjects/MZStore.woa/wa/viewSoftware?id=1439307856&mt=8',
  mydax: '#' //(no link yet)
}
